...
export default function ListeFlottes() {
  ...
  const all = useMemo(() => {
    ...
    const own = rapport.flottesJoueur.map(f => {
      const stats = calculateFleetCombatStats(f as FlotteJoueur, rapport.plansVaisseaux, global, f.lieutenant);
      return {
        ...f,
        ...stats,
        nbv: f.nbVso ?? 0,
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a: any, b: any) => {
      ...
      switch (sortKey) {
        ...
        case 'cdt': av = a.cdt === 'N/A' ? -1 : a.cdt ?? 0; bv = b.cdt === 'N/A' ? -1 : b.cdt ?? 0; break;
        default: av = 0; bv = 0;
      }
          ...
          <tbody>
            {pageItems.map((f: any, i) => (
              <tr key={`${f.type}-${f.num}-${i}`}>
                ...
                <td>{f.heros ?? '—'}</td>
                <td style={{ textAlign: 'right' }}>{f.cdt ?? '—'}</td>
